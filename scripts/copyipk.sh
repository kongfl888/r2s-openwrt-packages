...
mkdir -p ./r2srom/ipk/
mkdir -p ./r2srom/ipk/generic/
mkdir -p ./r2srom/ipk/packages/
...
if [ `find friendlywrt-rk3328/friendlywrt/bin/packages/* -name "*.ipk" | grep ".ipk" -c` -gt 0 ]; then
	mv -f friendlywrt-rk3328/friendlywrt/bin/targets/*/generic/packages/*.ipk ./r2srom/ipk/generic/ >/dev/null 2>&1 || echo "pack generic ipk fail" >> ipklost.txt
    mv -f friendlywrt-rk3328/friendlywrt/bin/packages/*/*/*.ipk ./r2srom/ipk/packages/ >/dev/null 2>&1 || echo "pack all ipk fail" >> ipklost.txt
fi
